...
export async function run() {
    try {
        ...
        if (response === 0) {
            tl.setResult(tl.TaskResult.Succeeded, 'GCPGraphSuccessNoAdditionalArgsL0 should have succeeded.');
        } else{
            tl.setResult(tl.TaskResult.Failed, 'GCPGraphSuccessNoAdditionalArgsL0 should have succeeded but failed.');
        }
    } catch(error) {
        tl.setResult(tl.TaskResult.Failed, 'GCPGraphSuccessNoAdditionalArgsL0 should have succeeded but failed.');
    }
